...
\begin{equation}
	\widehat{\beta}-\widehat{\beta}_{-S}
	=
	\frac{\sum_{s\in S}\widetilde{x}_s\widetilde{r}_s}
...
Under the positive-denominator condition, the fixed-\(k\) problem is a linear-fractional optimisation problem. For a Dinkelbach parameter \(\eta\), the transformed objective ranks observations by
\[
s_i(\eta)=w_i+\eta c_i.
\]
...
For fixed residualised inputs and the positive-denominator condition in \autoref{eq:positive_denominator}, this procedure identifies a global maximiser of the stated fixed-\(k\) linear-fractional objective. This is the scope of ``exact'' optimisation used throughout the thesis. It does not imply global optimisation of the coefficient obtained after re-estimating the complete multivariate model. Full-refit validation remains a separate operation. Numerical convergence and computational checks are reported in \autoref{app:numerical}.
