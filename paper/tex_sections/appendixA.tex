...
\subsection{Supplementary Script 04 Results}
\label{app:estimation-results}

\subsubsection{Distributional and Data-Generating-Process Variation}
\label{app:estimation-dgp}

The full absolute-bias distributions provide additional detail on the aggregate results reported in Section~\ref{subsec:estimation-performance}. MM and LTS remain concentrated near zero across the principal contamination mechanisms, while the diagnostic-removal estimators exhibit wider distributions in mechanism-specific settings (\autoref{fig:04A-absolute-bias}). MIS with oracle \(k\) shows particularly wide absolute-error distributions under vertical-outlier and good-leverage contamination. Under bad leverage, its distribution contracts substantially relative to OLS, consistent with the stronger alignment between the injected coalition and the target-coefficient objective.
...
\subsubsection{Oracle-\(k\) MIS under Bad Leverage}
...
Comparison with the classical deletion procedures is more heterogeneous. Under bad leverage with a normal predictor, MIS with oracle \(k\) often has lower mean absolute error than Cook's distance and DFBETAS, whereas contaminated-predictor designs frequently favour the classical deletion estimators (\autoref{fig:04A-mis-oracle-vs-classical}). This variation is important because influential-set localisation and post-deletion coefficient accuracy are different outcomes. Accurate recovery of the injected coalition need not minimise the error of the subsequent OLS refit.
...
\begin{figure}[htbp]
	...
	\caption{Relative mean absolute error of classical diagnostic deletion and MIS with oracle \(k\) under bad-leverage contamination. Values report \(\log_2(\mathrm{MAE}_{\mathrm{classical}}/\mathrm{MAE}_{\mathrm{MIS}})\); positive values indicate lower mean absolute error for MIS with oracle \(k\).}
	\label{fig:04A-mis-oracle-vs-classical}
\end{figure}

The detailed bad-leverage results illustrate this distinction directly (\autoref{tab:robust-bad-leverage-dgp-detail}). With normal predictors and normal errors, MIS with oracle \(k\) achieved \(100.0\%\) precision and mean absolute error \(0.00995\), compared with \(0.03610\) for Cook's distance and \(0.04415\) for DFBETAS. With contaminated predictors and normal errors, MIS precision remained high at \(95.9\%\), yet its mean absolute error was \(0.02231\), compared with \(0.00086\) for Cook's distance and \(0.00124\) for DFBETAS. High coalition recovery therefore establishes localisation accuracy without establishing optimal post-deletion estimation. The legacy GPD condition is excluded from this finite-moment table and is retained in the large-error diagnostic in \autoref{fig:04-mis-oracle-large-errors}.

\input{../../output/04_robust/tables/supplement/04_tabA3_bad_leverage_dgp_detail.tex}

\subsubsection{Runtime and Numerical Availability}
\label{app:estimation-computation}

The computational comparison shows that fixed-\(k\) MIS requires more time than the classical diagnostic-removal procedures but remains comparable with MM regression in these simulations. Mean MIS runtime ranges from \(1.469\) to \(1.655\) seconds across the four mechanisms, while MM ranges from \(1.506\) to \(1.845\) seconds (\autoref{tab:robust-runtime}). LTS requires approximately \(0.11\) seconds per draw, and the OLS and classical diagnostic procedures are substantially faster. The complete runtime distributions are shown in \autoref{fig:04A-runtime}.
...
Numerical availability was complete for every estimator in the renewed Script 04 output. All \(8400\) recorded rows for each method produced finite coefficient estimates, coverage indicators, and runtimes (\autoref{tab:robust-method-health}). The estimation differences reported in the main text therefore reflect statistical performance in the recorded designs rather than differential numerical non-completion.
...
\subsubsection{Correct-Model Calibration and Frozen Estimation}
...
The comparison of full OLS and full MM similarly shows that lower RMSE does not imply improved interval calibration in every environment. MM has lower RMSE in all eight frozen scenarios within each predictor-distribution stratum, but coverage is closer to \(95\%\) in none of the normal-\(X\) cases and in only four of eight cases under each of the mixed-normal and contaminated predictor designs (\autoref{tab:05-frozen-mm-win-counts}).
