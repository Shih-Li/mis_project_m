...
Block size relative to the deletion budget is the binding diagnostic for the block-maxima construction (\autoref{tab:02A-block-count-summary}). Block count alone understates the problem. Fifty blocks were retained at contamination proportions of \(0.5\%\) and \(1\%\), while the block count declined to approximately \(39\)--\(40\) at \(2.5\%\), \(19\) at \(5\%\), and \(9\) at \(10\%\). Block size falls alongside the count. The ratio of \(k\) to block size rises from roughly \(0.24\) at \(0.5\%\) contamination to about \(0.53\) at \(1\%\), and it reaches exactly \(1.00\) at \(2.5\%\) and above.

A ratio of one admits a single size-\(k\) subset per block. The blockwise value is then the influence of that whole block, and the fitted GEV describes ordinary block-level influence. Twelve of the twenty sample-size and contamination cells reach this ratio. Numerical convergence remained high throughout these degenerate cells (\autoref{tab:02A-evt-convergence-grid}), which confirms that successful GEV fitting establishes the numerical availability of a rejection calculation and nothing about the quality of the underlying extremal approximation. EVT results hold at \(0.5\%\) and are marginal at \(1\%\).
...
The two null experiments are not directly comparable, and their difference is informative. The algorithmic null experiment holds at least thirty observations in every block, so the ratio of \(k\) to block size stays between \(0.01\) and \(0.50\) across its grid. The distributional-recovery design constrains blocks only to hold \(k\) observations, and its ratio reaches one in most cells. Conservative rejection appears where the blockwise values are genuine maxima. Rejection near the nominal level appears where the construction has degenerated toward ordinary block-level influence. The near-nominal empirical size recorded in \autoref{tab:02-empirical-size-summary} should therefore not be read as evidence of correct EVT calibration.
...
\input{../../output/04_robust/tables/supplement/04_tab_design_audit.tex}
...
\input{../../output/04_robust/tables/supplement/04_tabA3_bad_leverage_by_predictor.tex}
...
\input{../../output/05_misspecification/tables/supplement/05_tabS02_size_by_x_n.tex}
...
\input{../../output/05_misspecification/tables/main/05_tab02_null_scaling_slopes.tex}
...
\input{../../output/05_misspecification/tables/supplement/05_tabS05_power_by_error.tex}

\input{../../output/05_misspecification/tables/supplement/05_tabS06_power_by_x.tex}

\input{../../output/05_misspecification/tables/supplement/05_tabS07_power_by_n.tex}

The environment-level heatmap shows that these pooled averages conceal meaningful heterogeneity across predictor and error distributions (\autoref{fig:05S-environment-heterogeneity}). At maximum simulated severity, several scenarios attain uniformly high power, whereas heteroskedasticity, omitted variables, and the invariant linear-endogeneity case retain substantial weak environments. The worst-environment summaries quantify this variation (\autoref{tab:05-worst-case-environment}): for example, maximum-severity threshold power remains at least \(95.4\%\) across the \(96\) environments, whereas omitted-variable power ranges from \(0.0\%\) to \(100.0\%\) and has a median of only \(7.2\%\).
...
\input{../../output/05_misspecification/tables/supplement/05_tabS08_worst_case_environment.tex}
...
\input{../../output/05_misspecification/tables/supplement/05_tabA04_boundary_mean.tex}

The detailed deletion-budget results show that the pooled operating characteristic does not imply a universally preferred \(k\) (\autoref{tab:05-budget-by-scenario} and \autoref{tab:05-budget-mean}). No budget dominates across scenarios. For example, missing-interaction reach rises from \(84.4\%\) at \(k/n=0.01\) to \(100.0\%\) at \(0.05\), whereas nonlinear-confounding and heterogeneous-slope reach declines at the \(10\%\) budget. Linear endogeneity remains undetected at every budget, and omitted-variable reach remains low even though it increases to \(19.8\%\) at \(10\%\). The fixed-\(k\) analysis therefore describes sensitivity to analyst-chosen budgets rather than recovery of a latent contamination proportion.

\input{../../output/05_misspecification/tables/main/05_tab_budget_by_scenario.tex}

\input{../../output/05_misspecification/tables/supplement/05_tabA05_budget_mean.tex}
...
\input{../../output/05_misspecification/tables/supplement/05_tabS09_correct_cutoff_audit.tex}
...
\input{../../output/05_misspecification/tables/supplement/05_tabA01_mm_correct_vs_wrong.tex}

The comparison of full OLS and full MM similarly shows that lower RMSE does not imply improved interval calibration in every environment. MM has lower RMSE in all eight recorded scenarios within each predictor-distribution stratum, but coverage is closer to \(95\%\) in none of the normal-\(X\) cases and in only four of eight cases under each of the mixed-normal and contaminated predictor designs ((\autoref{tab:05-frozen-ols-vs-mm} and \autoref{tab:05-frozen-mm-win-counts}).

\input{../../output/05_misspecification/tables/supplement/05_tabA02_ols_vs_mm.tex}

\input{../../output/05_misspecification/tables/supplement/05_tabA03_mm_win_counts.tex}

Finally, the deletion-arm results show why detection should not be equated with model repair. MIS deletion frequently increases the mean cell-level RMSE as the deletion fraction grows, particularly under mixed-normal and contaminated predictors. With contaminated predictors, mean cell RMSE for MIS deletion rises from \(5.697\) at a \(1\%\) budget to \(18.209\) at \(10\%\). The unchanged full OLS fit records \(2.296\) (\autoref{tab:05-frozen-rmse-mean-median}). Cook's distance and DFBETAS deletion move in the opposite direction, falling as the budget grows. The structural defect remains in the estimating model after deletion, so improved diagnostic localisation does not imply improved estimation.

\input{../../output/05_misspecification/tables/supplement/05_tabA06_frozen_rmse_mean.tex}
