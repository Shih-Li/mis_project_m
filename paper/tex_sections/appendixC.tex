...
The study-specific results also clarify the scope of the cross-study comparison in \autoref{subsec:application-sensitivity}. An early threshold crossing establishes that the audit identifies a feasible small subset capable of producing a large change in the selected point estimate. A later crossing indicates that the searched paths do not attain the same change at smaller deletion sizes. Neither result evaluates the experimental design as a whole, and a zero crossing does not imply that every finding in the source study is similarly sensitive. The empirical application therefore treats MIS as a targeted diagnostic of finite-sample observation sensitivity.
